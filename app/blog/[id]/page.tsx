...
export default function BlogPost() {
  ...
  useEffect(() => {
    ...
    if (currentPost) {
      const related = blogPosts.filter((p) => p.id !== id && p.category === currentPost.category).slice(0, 3)
      setRelatedPosts(related)

      // Find next posts (different from related posts and current post)
      const next = blogPosts
        .filter((p) => p.id !== id && !related.some((r) => r.id === p.id) && p.category !== currentPost.category)
