...
export default function InteractiveParticles() {
  ...
  useEffect(() => {
    ...
    function initParticles() {
      ...
      switch (devicePerformance.current) {
        ...
        case "medium":
          ...
          break
        case "high":
          particleCount = Math.min(Math.floor(window.innerWidth / 25), 40)
          break
      }
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particle) => {
        ...
        if (!isCurrentlyScrolling) {
          ...
          if (distance < 120) {
            ...
            const force = 0.05 * (1 - distance / 120)
      ...
      if (!isCurrentlyScrolling && devicePerformance.current !== "low") {
        ...
        const connectionLimit = Math.min(particlesRef.current.length, devicePerformance.current === "high" ? 20 : 10)
  ...
  return (
    <motion.canvas
      ref={canvasRef}
      className="fixed top-0 left-0 w-full h-full z-0 pointer-events-none interactive-particles"
      style={{
        position: "fixed",
        width: "100vw",
        height: "100vh",
